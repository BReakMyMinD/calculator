...
string calculator::getRPN(string &str) {
	getAllTokens(str);
	int code = sort();
	if (code == 1) {
...
int calculator::sort() {
	...
	for (auto& item : tokens) {
		...
		//	if(bufferStack.isEmpty() || isOpenedBracket) {
...
token calculator::count(string &str) {
	...
	while (!exitQueue.isEmpty()) {
		...
		if (buf_1.type != NUMBER) {
			...
			if (buf_1.value.find_first_of('+') != string::npos) {
				buf_1.value = to_string(stod(buf_3.value) + stod(buf_2.value));
				buf_1.type = NUMBER;
			}
			if (buf_1.value.find_first_of('-') != string::npos) {
				buf_1.value = to_string(stod(buf_3.value) - stod(buf_2.value));
			...
			if (buf_1.value.find_first_of('^') != string::npos) {
				buf_1.value = to_string(pow(stod(buf_3.value),stod(buf_2.value)));
				buf_1.type = NUMBER;
			}
		}
...
void calculator::getAllTokens(string &str) {
	token currentToken;
	string value;
